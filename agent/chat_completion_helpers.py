...
def interruptible_api_call(agent, api_kwargs: dict):
    ...
    # The scaled no-byte TTFB watchdog above catches dead streams that never
    # emit a first byte, but a request that emits SOME bytes and then wedges
    # (the issue-64507 symptom: vision-inflated request, worker idle, no
    # ended_at) is only reclaimed at the (high) stale floor. Add a flat,
    # finite hard ceiling on total request time that ALWAYS applies to
    # openai-codex requests regardless of the TTFB/stale interaction, so a
    # stalled request is recovered (retry loop / visible failure) instead of
    # hanging indefinitely. The default sits ABOVE the maximum stale floor
    # (1200s) so it never clamps an intentionally-raised timeout for healthy
    # large requests — it is a backstop against unbounded growth, not a
    # tighter limit. Tunable via HERMES_CODEX_HARD_TIMEOUT_SECONDS (set to 0
    # to disable the ceiling entirely; that restores the pre-fix behavior).
